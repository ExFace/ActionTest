...
class ActionTestApp extends App
{
    ...
    public function getTestStepsData(DataSheetInterface $input_data_sheet, array $columns_array)
    {
        ...
        if ($input_data_sheet->getMetaObject()->is('exface.ActionTest.TEST_CASE')) {
            $saved_test_data->getFilters()->addConditionFromString('TEST_CASE', implode($input_data_sheet->getMetaObject()->getUidAttribute()->getValueListDelimiter(), $input_data_sheet->getUidColumn()->getValues()), EXF_COMPARATOR_IN);
            ...
            $saved_test_data->getFilters()->addConditionFromString($saved_test_data->getMetaObject()->getUidAttributeAlias(), implode($saved_test_data->getMetaObject()->getUidAttribute()->getValueListDelimiter(), $input_data_sheet->getUidColumn()->getValues()), EXF_COMPARATOR_IN);
        } else {
